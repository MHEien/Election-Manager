        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header with-border">
              <a href="#reset" data-toggle="modal" class="btn btn-danger btn-sm btn-flat"><i class="fa fa-refresh"></i> Archive</a>
            </div>
            <div class="box-body">
                ...
                <tbody>
                  <?php 
                    $sql = "SELECT *, votes.id AS voteid FROM votes LEFT JOIN candidates ON candidates.id=votes.candidate_id LEFT JOIN positions ON positions.id=candidates.position_id WHERE positions.election_id = candidates.election_id AND positions.active = 'active'";
                      //remove duplicates from the $sql query, so that each voter is only counted once
